...
// Дисконнект-цикл (этап 6): сторона соперника и id текущего countdown-интервала.
const opp_side = 3 - my_side;
let graceTimer = null;        // setInterval id отсчёта до авто-форфейта (или null)
...
function renderHud(v) {
  ...
  // Партия не идёт → прятать «Сдаться» и гасить любой отсчёт авто-форфейта.
  const resignBtn = document.getElementById("resign-btn");
  if (resignBtn) resignBtn.hidden = v.status !== "active";
  if (finished) clearGrace();
}

// === 5b. Присутствие соперника + авто-форфейт (этап 6) =============
function clearGrace() {
  if (graceTimer !== null) {
...
function onPresence(p) {
  const oppOnline = !!(p.online && p.online[String(opp_side)]);
  const el = document.getElementById("presence");
  if (el) {
    el.hidden = false;
    el.classList.toggle("online", oppOnline);
    el.classList.toggle("offline", !oppOnline);
    el.textContent = oppOnline ? "● соперник в сети" : "● соперник отключился";
  }
  // Соперник вернулся или партия завершена → снять отсчёт.
  if (oppOnline || view.winner != null) {
    clearGrace();
    return;
  }
  // Офлайн + сервер прислал срок до форфейта → запустить обратный отсчёт.
  if (typeof p.grace_seconds === "number") {
    clearGrace();
    const note = document.getElementById("presence-note");
    let left = p.grace_seconds;
    const tick = () => {
      if (note) {
        note.hidden = false;
        note.textContent = left > 0
          ? `Соперник отключился. Победа будет присуждена вам через ${left} с…`
          : "Соперник не вернулся. Завершаем партию…";
      }
      if (left <= 0) { clearInterval(graceTimer); graceTimer = null; return; }
      left -= 1;
    };
    tick();
...
// Кнопка «Сдаться»: подтверждение → resign по сокету (исход придёт в state).
const resignBtn = document.getElementById("resign-btn");
if (resignBtn) {
  resignBtn.addEventListener("click", () => {
    if (view.winner != null) return;
    if (confirm("Сдаться? Победа будет засчитана сопернику.")) {
